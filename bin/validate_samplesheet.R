...
for(u in unique_sample){
  ...
  #sx_group <- unique(sx$group)
  #if(length(sx_group)>1)
  #  l_error[paste0("group_", u)] <- paste("Sample:", u, "has multiple different groups")
  
  # R1 and R2 must not have duplicates
  sx_r1 <- table(sx$r1)
  sx_r1w <- which(sx_r1>01)
  if(length(sx_r1w)>0) 
    l_error[paste0("r1_", u)] <- paste("Duplicated r1:", paste(names(sx_r1)[sx_r1w], collapse = ","))
  
  if(unique(sx$r2)[1]!=""){
    sx_r2 <- table(sx$r2)
    sx_r2w <- which(sx_r2>01)
    if(length(sx_r2w)>0) 
